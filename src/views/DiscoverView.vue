...
function requestStarpick(force = false) {
  void commonStore.loadStarpick(force).catch(() => undefined)
}
...
function requestHomepagePlaylists(force = false) {
  void commonStore.loadHomepagePlaylists(force).catch(() => undefined)
}

function requestRecommendPrograms(force = false) {
  void commonStore.loadRecommendPrograms(force).catch(() => undefined)
}

function requestNewestRadios(force = false) {
  void commonStore.loadNewestRadios(force).catch(() => undefined)
}
...
<template>
  <main class="discover-shell">
    ...
    <StarpickSection
      :comments="starpick"
      :error="starpickError"
      :loading="starpickLoading"
      @retry="requestStarpick(true)"
    />
    ...
    <PersonalizedSection
      empty-title="暂无首页歌单"
      error-title="首页歌单加载失败"
      eyebrow="Home"
      hint="来自 APP 首页推荐块"
      testid="home-playlists"
      title="首页歌单"
      :error="homepagePlaylistsError"
      :loading="homepagePlaylistsLoading"
      :playlists="homepagePlaylists"
      @retry="requestHomepagePlaylists(true)"
    />

    <DjProgramSection
      empty-title="暂无精选节目"
      error-title="精选节目加载失败"
      testid="home-programs"
      title="精选节目"
      :error="recommendProgramsError"
      :loading="recommendProgramsLoading"
      :programs="recommendPrograms"
      @retry="requestRecommendPrograms(true)"
    />

    <DjRadioRankSection
      empty-title="暂无最新电台"
      error-title="最新电台加载失败"
      testid="home-newest"
      title="最新电台"
      :error="newestRadiosError"
      :loading="newestRadiosLoading"
      :radios="newestRadios"
      @retry="requestNewestRadios(true)"
    />
