...
> 更新日期：`2026-08-30`<br>
> 文档版本：`0.33.0`<br>
> 对照提交：`a2d6039`（实施第 28 轮已在当前 HEAD）<br>
> 工作区：实施第 29 轮代码已落地，**尚未 commit / push**
...
根工程已经是可运行的 Bun + Vue 3.5 + TypeScript 6 + Vite 8 应用。Discover、播放器（含上一首/下一首、循环/随机）、歌单、MV、音乐馆、歌手馆筛选、歌手 MV/专辑/介绍 tab、推荐电台、电台大厅、搜索（单曲/歌单/歌手/专辑）、专辑详情、顶部应用壳和视频大厅已经形成闭环。下一轮应迁移 **静音或播放列表抽屉**。

## 2. 路线图对照

原计划的 P0–P7 仍然有效，但实际切片顺序已经偏离 [03-migration-roadmap.md](./03-migration-roadmap.md) 的 P4 清单。见 [D-017](./06-decision-log.md) 至 [D-039](./06-decision-log.md)。
...
| P4 功能切片 | 按垂直功能移植播放器级应用 | **进行中** | 实施第 4–29 轮 | Discover、播放器（含上一首/下一首、循环/随机）、歌单、MV、排行榜、分类、精选、歌手馆、歌手 MV/专辑/介绍 tab、电台节目、电台大厅、搜索多类型、专辑详情、应用壳和 `#/video` 完成；静音、播放列表抽屉未做 |
| P5 类型与依赖 | 严格类型、去掉冗余依赖 | **随切片推进** | 第 3–29 轮内嵌 | 尚无独立治理轮 |
...
| 实施第 29 轮 | 2026-08-30 | 歌手详情介绍 tab | 89 文件 / 362 测试 | **工作区未提交** |

第 29 轮文档对齐时的当前门禁输出：

```text
bun run test       89 files / 362 tests passed
...
第 29 轮已完成本地 mock API 浏览器 smoke，但未验证外部真实网易云 API。未 commit、未 push。

歌手介绍摘要：歌曲 / 专辑 / 视频 / 详情四个 tab。点详情才请求 `GET /artist/desc`。介绍段落用纯文本渲染，不走 HTML。无介绍时回退 `briefDesc`。不迁精选 tab、静音、播放列表抽屉。

本地 smoke 使用 Vite `127.0.0.1:48821` 和 mock API `127.0.0.1:48831`：Host → 歌手详情 → 详情 tab → 经历/代表作 → 重新配置。桌面 `1440×900` 与移动 `390×844`（详情 tab 可见）无横向溢出。控制台无应用错误。
...
```text
GET /artist/desc         歌手介绍段落 + briefDesc
Artist store             desc / loadDesc / clearDesc
ArtistDescSection        纯文本介绍，失败可重试
```
...
已通过第 29 轮当前门禁：89 个测试文件 / 362 个测试、两套 typecheck、330 modules build、frozen lock、audit 和 `git diff --check`。

仍存在、但不阻塞第 29 轮的缺口：Host 文案仍写 round 3、无 lint/E2E/CI。精选 tab、静音和播放列表抽屉未迁。

## 7. 建议的下一轮

**实施第 30 轮：静音或播放列表抽屉。**

歌手介绍已经接上。播放器还缺静音和队列抽屉。
...
> 状态更新（2026-08-30）：第 28 轮已在当前 HEAD `a2d6039` 完成。第 29 轮已在工作区完成歌手介绍 tab；静音和播放列表抽屉仍未迁。
