...
> 更新日期：`2026-08-29`<br>
> 文档版本：`0.20.0`<br>
> 对照提交：`11535de`（实施第 15 轮已在当前 HEAD）<br>
> 工作区：实施第 16 轮代码已落地，**尚未 commit / push**
...
根工程已经是可运行的 Bun + Vue 3.5 + TypeScript 6 + Vite 8 应用。Discover、最小播放器、歌单详情、MV 播放、音乐馆排行榜、分类歌单、精选、歌手详情、歌手馆列表和推荐电台已经形成闭环。下一轮应迁移 **搜索**；应用壳、播放器增强、歌手馆分类/字母筛选和电台大厅后置。

## 2. 路线图对照

原计划的 P0–P7 仍然有效，但实际切片顺序已经偏离 [03-migration-roadmap.md](./03-migration-roadmap.md) 的 P4 清单。见 [D-017](./06-decision-log.md) 至 [D-026](./06-decision-log.md)。
...
| P4 功能切片 | 按垂直功能移植播放器级应用 | **进行中** | 实施第 4–16 轮 | Discover、播放器、歌单详情、MV、排行榜、分类歌单、精选、歌手详情、歌手馆（语种）和推荐电台完成；壳、搜索、播放器增强、电台大厅、歌手馆分类/字母筛选未做 |
| P5 类型与依赖 | 严格类型、去掉冗余依赖 | **随切片推进** | 第 3–16 轮内嵌 | 尚无独立治理轮 |
...
| 实施第 16 轮 | 2026-08-29 | 精选推荐电台 + `#/dj?id=` 节目详情 | 64 文件 / 223 测试 | **工作区未提交** |

第 16 轮文档对齐时的当前门禁输出：

```text
bun run test       64 files / 223 tests passed
bun run typecheck  PASS
bun run build      267 modules transformed, dist/ 输出
bun install --frozen-lockfile --dry-run  PASS
bun audit          No vulnerabilities found (checked 185 packages)
git diff --check   PASS
```

第 16 轮已完成本地 mock API 浏览器 smoke，但未验证外部真实网易云 API。未 commit、未 push。

电台摘要：legacy 精选 `DjProgram` 请求 `GET /personalized/djprogram`。本轮把推荐电台接回精选，卡片进入 `#/dj?id=`（保留 legacy `dj` 名称）。详情 `GET /dj/program/detail` 展示封面、电台名、主播、简介；`mainSong` 走已有 Player。缺失或无效 ID 只清详情字段。Host 重新配置会 `djStore.reset()`。不迁空的电台大厅 Banner/分类，也不复用 legacy 点进 `video` 的未完成行为。Discover 本来就没有电台区块。

本地 smoke 使用 Vite `127.0.0.1:46179` 和 mock API `127.0.0.1:46515`：Host → Discover → 音乐馆精选，可见「深夜民谣」「秋日电台」；点击进入 `#/dj?id=901`；「播放节目」后出现「正在播放“晚风来信”。」和 PlayerBar。Host 重新配置后 503 显示 `mock radio unavailable`，重试恢复。桌面 `1440×900` 与移动 `390×844` 无横向溢出。成功路径控制台无应用错误；503 验证期间浏览器记录了一次资源 503。
...
```text
PickedView 推荐电台     GET /personalized/djprogram
DjView                  GET /dj/program/detail
                        mainSong → Player.play
Dj store                loadPrograms / load / resetDetail
...
| 产品面 | legacy | 新工程 | 下一动作 |
| --- | --- | --- | --- |
| 精选电台 | 推荐节目卡片（点进未完成的 `video`） | 推荐节目 + `#/dj?id=` 详情 + 播放 | 完成最小闭环 |
| 电台大厅 | 空页 + 菜单项 | 无大厅 Banner/分类 | 后续增强 |
| 搜索 | 有 | 未迁 | **下一轮优先** |
| 应用壳 / 播放器增强 | 有 | 无或不完整 | 后置 |

## 6. 质量与文档缺口

已通过第 16 轮当前门禁：64 个测试文件 / 223 个测试、两套 typecheck、267 modules build、frozen lock、audit 和 `git diff --check`。

仍存在、但不阻塞第 16 轮的缺口：数字工具无专用单测、Host 文案仍写 round 3、无 lint/E2E/CI、搜索/应用壳未迁、播放器无进度音量。专辑、歌手 MV tab、歌手馆分类/字母筛选和电台大厅未迁。

## 7. 建议的下一轮

**实施第 17 轮：搜索。**

输入已经具备：Host、路由、Player、歌单/歌手/MV/电台详情可作为结果落地页。

本轮应做：

1. 最小可见搜索入口（热搜或关键词结果择一形成可点闭环）；
2. 结果进入已有详情或播放；
3. 保留 loading/error/retry 和移动端布局。

本轮不应做：电台大厅、歌手馆分类/字母全套筛选、专辑/视频 tab、Tailwind 4、播放器增强、CI。
...
> 状态更新（2026-08-29）：第 15 轮已在当前 HEAD `11535de` 完成。第 16 轮已在工作区完成精选推荐电台与节目详情；搜索仍未迁。下一轮优先迁移搜索。
