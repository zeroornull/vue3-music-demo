...
> 更新日期：`2026-08-29`<br>
> 文档版本：`0.18.0`<br>
> 对照提交：`5fa2d24`（实施第 13 轮已在当前 HEAD）<br>
> 工作区：实施第 14 轮代码已落地，**尚未 commit / push**
...
根工程已经是可运行的 Bun + Vue 3.5 + TypeScript 6 + Vite 8 应用。Discover、最小播放器、歌单详情、MV 播放、音乐馆排行榜、分类歌单、精选和歌手详情已经形成闭环。下一轮应迁移 **歌手馆列表**；电台、应用壳和播放器增强后置。

## 2. 路线图对照

原计划的 P0–P7 仍然有效，但实际切片顺序已经偏离 [03-migration-roadmap.md](./03-migration-roadmap.md) 的 P4 清单。见 [D-017](./06-decision-log.md) 至 [D-024](./06-decision-log.md)。
...
| P4 功能切片 | 按垂直功能移植播放器级应用 | **进行中** | 实施第 4–14 轮 | Discover、播放器、歌单详情、MV、排行榜、分类歌单、精选和歌手详情完成；壳、歌手馆、搜索、播放器增强、电台未做 |
| P5 类型与依赖 | 严格类型、去掉冗余依赖 | **随切片推进** | 第 3–14 轮内嵌 | 尚无独立治理轮 |
...
| 实施第 14 轮 | 2026-08-29 | 歌手详情 `/artist/detail` + 热门歌曲 | 54 文件 / 192 测试 | **工作区未提交** |

第 14 轮文档对齐时的当前门禁输出：

```text
bun run test       54 files / 192 tests passed
bun run typecheck  PASS
bun run build      244 modules transformed, dist/ 输出
bun install --frozen-lockfile --dry-run  PASS
bun audit          No vulnerabilities found (checked 185 packages)
git diff --check   PASS
```

第 14 轮已完成本地 mock API 浏览器 smoke，但未验证外部真实网易云 API。未 commit、未 push。

歌手详情摘要：legacy `artistDetail?id=`。`GET /artist/detail` 提供封面、名称、简介和计数；`GET /artist/songs` 按 `hot` 分页，每页 10 首（legacy 为 20）。歌曲点击接入已有 Player。歌单行里的歌手名进入详情。`/music/artist` 歌手馆仍是边界页。Host 重新配置会 `artistStore.reset()`。

本地 smoke 使用 Vite `127.0.0.1:45967` 和 mock API `127.0.0.1:48503`：Host → Discover → `#/playlist?id=101` → 歌手名「林间电台」→ `#/artistDetail?id=401`；播放「晚风来信」后 PlayerBar 出现。503 显示 `mock artist unavailable`，重试恢复。缺少 `id` 显示明确空态。桌面 `1440×900` 与移动 `390×844` 无横向溢出。成功路径控制台无应用错误；503 验证期间浏览器记录了一次资源 503。
...
```text
PlaylistSongItem 歌手名 ──► artistDetail?id=
ArtistView               GET /artist/detail
                         GET /artist/songs (order=hot, limit 10)
                         play / playAll → Player
```
...
| 歌手馆 | 筛选 + artist/list | 边界页 | **下一轮优先** |
| 精选电台 | 有 | 未迁 | 后置 |
| 应用壳 / 搜索 / 播放器增强 | 有 | 无或不完整 | 后置 |

## 6. 质量与文档缺口

已通过第 14 轮当前门禁：54 个测试文件 / 192 个测试、两套 typecheck、244 modules build、frozen lock、audit 和 `git diff --check`。

仍存在、但不阻塞第 14 轮的缺口：数字工具无专用单测、Host 文案仍写 round 3、无 lint/E2E/CI、歌手馆/电台未迁、播放器无进度音量。专辑和歌手 MV tab 未迁。

## 7. 建议的下一轮

**实施第 15 轮：歌手馆列表。**

输入已经具备：`artistDetail?id=` 详情、卡片/loading/retry 模式。

本轮应做：

1. `/music/artist` 最小可见歌手列表（可先固定一组筛选，而不是一次迁完所有字母/地区控件）；
2. 点击进入已有歌手详情；
3. 保留 loading/error/retry 和移动端布局。

本轮不应做：电台、Tailwind 4、播放器增强、CI。
...
> 状态更新（2026-08-29）：第 13 轮已在当前 HEAD `5fa2d24` 完成。第 14 轮已在工作区完成歌手详情；歌手馆仍是边界页。下一轮优先迁移歌手馆列表。
