...
> 更新日期：`2026-08-29`<br>
> 文档版本：`0.15.0`<br>
> 对照提交：`b37d1db`（实施第 10 轮已在当前 HEAD）<br>
> 工作区：实施第 11 轮代码已落地，**尚未 commit / push**
...
根工程已经是可运行的 Bun + Vue 3.5 + TypeScript 6 + Vite 8 应用。Discover、最小播放器、歌单详情、MV 播放和音乐馆排行榜已经形成闭环。下一轮应迁移 **分类歌单**；精选、歌手、应用壳和播放器增强后置。

## 2. 路线图对照

原计划的 P0–P7 仍然有效，但实际切片顺序已经偏离 [03-migration-roadmap.md](./03-migration-roadmap.md) 的 P4 清单。见 [D-017](./06-decision-log.md) 至 [D-021](./06-decision-log.md)。
...
| P4 功能切片 | 按垂直功能移植播放器级应用 | **进行中** | 实施第 4–11 轮 | Discover、播放器、歌单详情、MV 和排行榜完成；壳、精选/歌手/分类、搜索、播放器增强未做 |
| P5 类型与依赖 | 严格类型、去掉冗余依赖 | **随切片推进** | 第 3–11 轮内嵌 | 尚无独立治理轮 |
...
| 实施第 11 轮 | 2026-08-29 | 音乐馆路由骨架 + 排行榜 `/toplist/detail` | 39 文件 / 146 测试 | **工作区未提交** |

第 11 轮文档对齐时的当前门禁输出：

```text
bun run test       39 files / 146 tests passed
bun run typecheck  PASS
bun run build      210 modules transformed, dist/ 输出
bun install --frozen-lockfile --dry-run  PASS
bun audit          No vulnerabilities found (checked 185 packages)
git diff --check   PASS
```

第 11 轮已完成本地 mock API 浏览器 smoke，但未验证外部真实网易云 API。未 commit、未 push。

排行榜摘要：`/toplist/detail` 的前 4 条为官方榜（封面 + 前 3 首），其余为特色榜封面网格；点击进入已有 `playlist?id=`。`/music` 仍重定向到 `picked`。精选/歌手/分类是明确边界页。Host 重新配置会 `musicStore.reset()`。

本地 smoke 使用 Vite `127.0.0.1:48625` 和 mock API `127.0.0.1:45907`：Host → Discover → `#/music/picked` 边界 → `#/music/toplist`；`/toplist/detail` 200；4 张官方榜 + 2 张特色榜；点击飙升榜进入 `#/playlist?id=19723756`，detail/track 200。503 显示 `mock toplist unavailable`，重试恢复。桌面 `1440×900` 与移动 `390×844` 无横向溢出；当前 tab `aria-current="page"` 为「排行」。成功路径控制台无应用错误；503 验证期间浏览器记录了一次资源 503。
...
| `#/music/category` | `category` | 边界页 | 未迁内容 |
...
```text
MusicView ── 栏目导航
  ├─ picked / artist / category  → 边界页
  └─ TopListPage                 GET /toplist/detail → music.topLists
                                 OfficialChartCard / ChartCoverCard → playlist?id=
```
...
| 精选 | Banner/Video/Dj/Mv 拼盘 | 边界页 | 后续 |
| 歌手 | 筛选 + artistDetail | 边界页 | 需先迁歌手详情 |
| 分类歌单 | 热门分类 + highquality 列表 | 边界页 | **下一轮优先** |
| 应用壳 / 搜索 / 播放器增强 | 有 | 无或不完整 | 后置 |

## 6. 质量与文档缺口

已通过第 11 轮当前门禁：39 个测试文件 / 146 个测试、两套 typecheck、210 modules build、frozen lock、audit 和 `git diff --check`。

仍存在、但不阻塞第 11 轮的缺口：数字工具无专用单测、Host 文案仍写 round 3、无 lint/E2E/CI、精选/歌手/分类未迁、播放器无进度音量。

## 7. 建议的下一轮

**实施第 12 轮：分类歌单。**

输入已经具备：音乐馆壳、`playlist?id=` 详情、卡片/loading/retry 模式。

本轮应做：

1. 热门分类或 `/top/playlist/highquality` 的最小可见列表；
2. 点击进入已有歌单详情；
3. 保留 loading/error/retry 和移动端网格。

本轮不应做：歌手详情、精选拼盘、Tailwind 4、播放器增强、CI。
...
> 状态更新（2026-08-29）：第 10 轮已在当前 HEAD `b37d1db` 完成。第 11 轮已在工作区完成音乐馆路由骨架和排行榜；精选/歌手/分类仍是边界页。下一轮优先迁移分类歌单。
