...
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 | Discover 四个推荐内容模块、播放器最小闭环和歌单详情已接入；应用壳、搜索、MV 播放和播放器增强后置 |
...
推荐顺序（**实际执行已偏离**，见 D-017：第 4–9 轮先完成 Discover 四块内容、播放器最小闭环和完整歌单详情，应用壳和菜单后置；下一轮改为 MV 播放）：
