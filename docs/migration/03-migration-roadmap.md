...
> 本文描述未来执行顺序。当前文档轮次不会运行这些命令或移动文件。
...
| --- | --- | --- | --- |
| P0 | 保护文档和采集行为基线 | 基线清单、文档安全边界 | 确认不会被 build 清空 |
| P1 | 把旧工程移动到 `legacy/` | 可追溯的旧工程目录 | 移动白名单核对完成 |
| P2 | 建立现代空壳 | Bun + Vue + TS + Vite 最小应用 | 空壳 typecheck/build/dev 通过 |
| P3 | 建立基础设施 | Router、Pinia、API、主题、自动组件 | 基础设施测试和 smoke 通过 |
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 |
| P5 | 类型与依赖治理 | 严格类型、移除冗余依赖 | 类型/lint 无未解释错误 |
| P6 | Tailwind 4 和视觉收敛 | 新样式入口和视觉证据 | 关键页面视觉验收通过 |
| P7 | 发布与文档闭环 | `dist/`、部署配置、学习总结 | 完整 `check` 和部署 smoke 通过 |
