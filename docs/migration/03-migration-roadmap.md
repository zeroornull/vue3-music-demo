...
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 | Discover 四个推荐内容模块与播放器最小闭环已接入；应用壳、详情、搜索和播放器增强后置 |
...
推荐顺序（**实际执行已偏离**，见 D-017：第 4–8 轮先完成 Discover 四块内容和播放器最小闭环，应用壳和菜单后置；下一轮改为完整歌单详情）：
