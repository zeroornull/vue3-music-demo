...
| 阶段 | 目标 | 主要产物 | 进入下一阶段的条件 | 2026-08-28 状态 |
| --- | --- | --- | --- | --- |
...
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 | Discover 四块内容完成；播放器未开始 |
...
推荐顺序（**实际执行已偏离**，见 D-017：第 4–7 轮先完成 Discover 四块内容，应用壳和菜单后置；下一轮改为播放器最小闭环）：
