...
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 | Discover、播放器（含上一首/下一首、循环/随机、静音、队列抽屉、歌词）、歌单、MV、排行榜、分类、精选、歌手馆筛选、歌手 MV/专辑/介绍 tab、电台节目、电台大厅、搜索（单曲/歌单/歌手/专辑）、专辑详情（含介绍 tab）、应用壳和 `#/video`（含分页）已接入；专辑评论、翻译歌词、电台分类与视频全部分类弹出层后置 |
