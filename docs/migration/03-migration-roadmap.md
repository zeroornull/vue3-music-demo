...
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 | Discover、播放器（含上一首/下一首、循环/随机）、歌单、MV、排行榜、分类、精选、歌手馆筛选、歌手 MV/专辑 tab、电台节目、电台大厅、搜索（单曲/歌单/歌手/专辑）、专辑详情、应用壳和 `#/video` 已接入；歌手详情 tab 后置 |
