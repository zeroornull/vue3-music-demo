...
| P4 | 分功能移植 | 可逐步使用的新音乐播放器 | 每个切片独立验收 | Discover 四个推荐内容模块、播放器最小闭环、歌单详情和 MV 播放已接入；应用壳、音乐馆、搜索和播放器增强后置 |
...
推荐顺序（**实际执行已偏离**，见 D-017：第 4–10 轮先完成 Discover 四块内容、播放器最小闭环、完整歌单详情和 MV 播放，应用壳和菜单后置；下一轮改为音乐馆）：
