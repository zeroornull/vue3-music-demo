...
- **状态**：已验证
- **日期**：`2026-08-31`
- **决策**：第 38 轮用同一条 `GET /lyric` 的 `romalrc.lyric`。按时间戳贴到原文下面（有翻译时贴在翻译下面），文本节点渲染。没有罗马音轨就不显示。`nolyric` 仍返回空。Host `reset()` 清掉罗马音。切歌仍丢掉进行中的旧请求。
- **原因**：罗马音已经在歌词响应里，不必另开接口。不迁逐字卡拉 OK、不迁空评论、不迁付费电台。
- **本轮不做**：逐字卡拉 OK、评论/收藏、付费电台、Header 弹出层。

### D-049：播放器本轮只补逐字卡拉 OK

- **状态**：已验证
- **日期**：`2026-08-31`
- **决策**：第 39 轮用同一条 `GET /lyric` 的 `yrc.lyric`。解析 `[startMs,dur](startMs,dur,0)字`，按行时间戳贴到原文。有逐字轨时原文拆成文本节点，当前字随进度高亮。没有 `yrc` 或对不上时间就保持整行原文。`nolyric` 仍返回空。Host `reset()` 清掉。不解析 `klyric` / JSON yrc。
- **原因**：逐字轨已经在歌词响应里，不必另开接口。legacy 没有卡拉 OK 实现，用文本节点高亮即可闭环。不迁空评论、不迁付费电台。
- **本轮不做**：`klyric`、JSON yrc、评论/收藏、付费电台、Header 弹出层。

### D-050：付费电台只标状态，不接购买

- **状态**：已验证
- **日期**：`2026-08-31`
- **决策**：第 40 轮从现有 `/dj/radio/hot`、`/dj/detail`、`/dj/program`、`/dj/program/detail` 读取 `feeScope` / `fee` / `programFeeType`。大于 0 视为付费。大厅卡片标「付费」。详情页写明不支持购买。付费节目不做链接、不能播放。Host `reset()` 仍清掉电台缓存。
- **原因**：没有登录，也接不了 `/dj/paygift` 购买。legacy 大厅是空页，没有购买 UI 可抄。先挡住误点播放。
- **本轮不做**：登录、购买、`/dj/paygift`、专辑空评论、Header 弹出层。

### D-051：顶栏搜索弹出层用原生控件

- **状态**：已验证
- **日期**：`2026-08-31`
- **决策**：第 41 轮在 AppShell 加原生搜索框。焦点打开面板：空关键词显示热搜，输入走已有 `/search/suggest`。单曲播放，歌单/歌手/专辑打开已有详情。Enter 进 `#/search?q=`。Escape 和点外面关闭。不用 Element Plus，不加 lodash。Host `reset()` 仍清搜索缓存。
- **原因**：legacy 用 `el-popover`。搜索页已经有热搜和多类型结果，顶栏只是入口。不迁用户信息、主题切换、邮件图标。
- **本轮不做**：登录、主题、用户信息、Tailwind 4、CI、Element Plus。

### D-052：已有详情页的 Banner 直接跳转

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 42 轮把 Discover 和精选 Banner 接到已有详情。`targetType` 1 播放，10 `#/album?id=`，1000 `#/playlist?id=`，1004 `#/mvDetail?id=`。电台大厅共用 `resolveBannerTarget`。未知类型仍显示后续切片提示。id 必须是正整数。不打开外链。
- **原因**：电台大厅第 23 轮已经能跳专辑/歌单/MV。Discover 和精选还停在占位提示。文档里的独立 P5 轮没有未解释的类型错误或可删依赖，本轮改接产品路径。
- **本轮不做**：外链 Banner、电台/视频类型扩展、登录、Tailwind 4、CI、独立 P5 治理。

### D-053：顶栏视频入口用已有大厅

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 43 轮在 AppShell 加「视频」，指向已有 `#/video`。`video` 和 `videoDetail` 的 `meta.menu` 改为 `video`，详情页仍标「视频」。MV 和歌单继续 `discover`。电台不进顶栏。
- **原因**：legacy 在线音乐菜单有视频。大厅第 27 轮已经能打开，只差壳入口。
- **本轮不做**：顶栏电台、登录、主题、Tailwind 4、CI、独立 P5 治理。

### D-054：Host 文案不再写迁移轮次

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 44 轮把 Host 表单 eyebrow 改成 API Host。`#/migration` 改成 API 状态页，只显示已保存地址和重新配置。404 链接改成返回推荐页。路径 `#/migration` 保留。Host 仍用 `/banner` 探活。
- **原因**：用户一打开仍看到 Migration round 3，已经不对应当前应用。
- **本轮不做**：主题、登录、Tailwind 4、CI、独立 P5 治理。

### D-055：主题用 data-theme 和 CSS 变量，不用 Tailwind 4

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 45 轮按 legacy Header 写 `html data-theme`。主题存在 `THEME`。AppShell 切换深色/浅色。`:root` 和 `[data-theme=dark]` 提供颜色变量。壳、Host、推荐页标题改用变量。内容卡片本轮不改。播放条保持深色底。Host 重新配置不清主题。
- **原因**：P3 主题未迁。P6 才是 Tailwind 4。先接通切换和壳层颜色。
- **本轮不做**：Tailwind 4、全站卡片换色、登录、CI、独立 P5 治理。

### D-056：内容卡片跟 CSS 变量，播放条仍深色

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 46 轮把 Discover 空状态和同色内容卡片、队列/歌词层接到已有主题变量，并增加 `--color-well`、`--color-danger-border`。播放条保持深色底。MV 播放画布保持 `#101826`。封面叠字保持白色。不用 Tailwind 4。
- **原因**：第 45 轮只改了壳和 Host，深色下卡片仍是浅色常量。
- **本轮不做**：播放条换色、登录、Tailwind 4、CI、独立 P5 治理、专辑空评论。

### D-057：歌曲正整数 mv 接到歌单行 MV 链接

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 47 轮把歌曲 payload 的正整数 `mv`（搜索 suggest 为 `mvid`）写入 `Song.mv`。`PlaylistSongItem` 显示 MV 链接到 `#/mvDetail?id=`，点击不播放。零、小数、缺省不显示。播放列表抽屉和推荐新歌卡片本轮不改。播放条保持深色。
- **原因**：legacy 歌单行有 MV 图标；新工程模型没有 `mv`，行上进不了已有 MV 页。
- **本轮不做**：队列 MV、新歌卡片 MV、顶栏搜索弹出层 MV、登录、Tailwind 4、CI、专辑空评论。

### D-058：队列和新歌卡片的 MV 链接是歌单行的兄弟控件

- **状态**：已验证
- **日期**：`2026-09-01`
- **决策**：第 48 轮在播放列表抽屉和推荐新歌卡片上，把 MV 做成播放按钮的兄弟 `RouterLink`，指向 `#/mvDetail?id=`。点击不播放。新歌 `mvid` 写入 `song.mv`。Discover 点播把正整数 `mv` 传进 `player.play`。队列里点 MV 会关闭抽屉。播放条保持深色。
- **原因**：第 47 轮只接通了 `PlaylistSongItem`。
- **本轮不做**：顶栏搜索弹出层 MV、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-059：顶栏搜索弹出层的 MV 链接同样是播放按钮的兄弟控件

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 49 轮在 HeaderSearchPop 单曲结果上把 MV 做成播放按钮的兄弟 `RouterLink`，指向 `#/mvDetail?id=`。点击不播放，并 `close()` 弹出层。播放条保持深色。
- **原因**：歌单行、队列、新歌卡片已能进 MV 页，顶栏弹出层还是只播放。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-060：歌单行专辑名接到已有专辑页

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 50 轮把 `PlaylistSongItem` 的专辑名做成播放按钮的兄弟 `RouterLink`，指向 `#/album?id=`。正整数专辑 id 才显示链接；零或缺省仍是文本。点击不播放。歌单、专辑、歌手热门和搜索单曲共用这一行。播放条保持深色。
- **原因**：legacy `SongListItem` 点专辑名进专辑页；新工程专辑名只是文字，而 `#/album` 已经能用。
- **本轮不做**：新歌卡片专辑链接、队列专辑、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-061：播放条封面用当前曲 picUrl，条本身仍深色

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 51 轮在 PlayerBar 显示 44×44 封面。优先 `Song.picUrl`，否则 `album.picUrl`；空白或缺省用占位，不渲染空 `src`。封面不是链接。播放条背景保持 `#172033`。不引入 legacy OpticalDisk 资源。
- **原因**：legacy `PlayerSong` 有封面；新工程 `normalizeSong` 已有 `picUrl`，播放条只写了歌名和歌手。
- **本轮不做**：封面点进专辑、播放条换色、登录、Tailwind 4、CI、专辑空评论、新歌卡片专辑。

### D-062：新歌卡片专辑名是播放按钮的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 52 轮把 NewSongCard 的专辑名做成播放按钮的兄弟 `RouterLink`，指向 `#/album?id=`。正整数专辑 id 才显示链接；零或缺省仍是按钮内文本。点击不播放。播放条保持深色。
- **原因**：专辑名原先在播放按钮里，点「专辑」会播歌；`#/album` 和歌单行专辑链接已经能用。
- **本轮不做**：队列专辑、封面点进专辑、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-063：播放条封面接到已有专辑页

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 53 轮把 PlayerBar 封面（有图或占位）包进 `RouterLink`，正整数 `album.id` 指向 `#/album?id=`。点击不切换播放。没有专辑 id 时封面仍是普通图。播放条背景保持 `#172033`。
- **原因**：第 51 轮只显示封面；`#/album` 和歌单/新歌专辑入口已经能用。
- **本轮不做**：队列专辑、新歌卡片歌手、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-064：新歌卡片歌手名是播放按钮的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 54 轮把 NewSongCard 的歌手名移出播放按钮，正整数 `artist.id` 做成兄弟 `RouterLink`，指向 `#/artistDetail?id=`。点击不播放。零或缺省 id 仍是文本。卡片边框包住标题和歌手行。播放条保持深色。
- **原因**：歌手名原先在播放按钮里，点歌手会播歌；`#/artistDetail` 和歌单行歌手链接已经能用。
- **本轮不做**：队列专辑、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-065：播放条歌手名接到已有歌手详情页

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 55 轮把 PlayerBar 当前曲歌手名做成 `RouterLink`，正整数 `artist.id` 指向 `#/artistDetail?id=`。点击不切换播放。零或缺省 id 仍是文本。空名字显示「未知歌手」。播放条背景保持 `#172033`。
- **原因**：歌手名原先是纯文本；`#/artistDetail` 和歌单行、新歌卡片歌手链接已经能用。
- **本轮不做**：队列歌手、队列专辑、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-066：队列歌手名是播放按钮的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 56 轮把 PlayerQueueDrawer 的歌手名移出播放按钮，正整数 `artist.id` 做成兄弟 `RouterLink`，指向 `#/artistDetail?id=`。点击不播放并 `closeQueue()`。零或缺省 id 仍是文本。空名字显示「未知歌手」。当前行高亮包住标题和歌手行。播放条背景保持 `#172033`。
- **原因**：歌手名原先在播放按钮里，点歌手会播歌；`#/artistDetail` 和歌单行、新歌卡片、播放条歌手链接已经能用。
- **本轮不做**：队列专辑、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-067：队列专辑名是播放按钮的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 57 轮把 PlayerQueueDrawer 的专辑名做成兄弟 `RouterLink`，正整数 `album.id` 指向 `#/album?id=`。点击不播放并 `closeQueue()`。零或缺省 id 仍是文本。空名字显示「未知专辑」。专辑和 MV 叠在行右侧。播放条背景保持 `#172033`。
- **原因**：队列原先没有独立专辑入口；`#/album` 和歌单行、新歌卡片、播放条封面专辑链接已经能用。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-068：顶栏搜索单曲歌手名是播放按钮的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 58 轮把 HeaderSearchPop 单曲行的歌手名做成兄弟 `RouterLink`，正整数 `artist.id` 指向 `#/artistDetail?id=`。点击不播放并 `close()` 弹出层。零或缺省 id 仍是文本。空名字显示「未知歌手」。标题和歌手行包在 `.hit-main` 里。播放条背景保持 `#172033`。
- **原因**：单曲行原先只有歌名和 MV；suggest 歌曲已有 `artists`，`#/artistDetail` 和歌单行、新歌卡片、播放条、队列歌手链接已经能用。
- **本轮不做**：顶栏搜索单曲专辑、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-069：顶栏搜索单曲专辑名是播放按钮的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 59 轮把 HeaderSearchPop 单曲行的专辑名做成兄弟 `RouterLink`，正整数 `album.id` 指向 `#/album?id=`。点击不播放并 `close()` 弹出层。零或缺省 id 仍是文本。空名字显示「未知专辑」。专辑和 MV 叠在行右侧。播放条背景保持 `#172033`。
- **原因**：单曲行原先没有独立专辑入口；suggest 歌曲已有 `album`，`#/album` 和歌单行、新歌卡片、播放条、队列专辑链接已经能用。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-070：播放条当前曲 MV 接到已有 MV 页

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 60 轮把 PlayerBar 当前曲的正整数 `mv` 做成标题旁的兄弟 `RouterLink`，指向 `#/mvDetail?id=`。点击不切换播放。零或缺省不显示。播放条背景保持 `#172033`。
- **原因**：歌单行、队列、新歌卡片和顶栏搜索已经能打开 MV；播放条当前曲已有 `Song.mv`，但没有入口。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-071：MV 卡片歌手名是封面链接的兄弟链接

- **状态**：已验证
- **日期**：`2026-09-02`
- **决策**：第 61 轮把 MvCard 的歌手名移出封面 `RouterLink`，正整数 `artist.id` 做成兄弟链接，指向 `#/artistDetail?id=`。点击不打开 MV。零或缺省 id 仍是文本。没有 `artists` 时，正整数 `artistId` 配 `artistName` 仍可链接，否则显示「未知艺人」。播放条背景保持 `#172033`。
- **原因**：歌手名原先在 MV 封面上，点歌手会进 MV；`#/artistDetail` 和歌单行、新歌卡片、播放条、队列、顶栏搜索歌手链接已经能用。
- **本轮不做**：MV 详情页歌手、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-072：MV 详情页头歌手接到已有歌手页

- **状态**：已验证
- **日期**：`2026-09-03`
- **决策**：第 62 轮把 MvView 页头的歌手名做成 `RouterLink`，正整数 `artist.id` 指向 `#/artistDetail?id=`。有具名 `artists[]` 时优先用列表，不再先用 `artistName` 拼串。零或缺省 id 仍是文本。没有 `artists` 时，正整数 `artistId` 配 `artistName` 仍可链接，否则隐藏。个性化缓存未命中不显示「未知艺人」。播放条背景保持 `#172033`。
...
### D-074：MV 详情页用 `/mv/detail` 补标题和歌手

- **状态**：已验证
- **日期**：`2026-09-03`
- **决策**：第 64 轮接入 `/mv/detail?mvid=`。URL 成功后异步拉详情，失败不挡住播放、不写 `error`。个性化缓存优先于详情。同一 id 若 URL 已缓存而详情为空，再试一次详情。播放条背景保持 `#172033`。
- **原因**：第 62 轮页头歌手只在推荐 MV 缓存命中时有数据；歌单/播放条/队列/搜索进 MV 会落到 `MV #id`。接口和页头链接已经能用。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、歌单行歌手 testid 对齐、相关 MV、评论。

### D-075：MV 详情页用 `/simi/mv` 接相关视频

- **状态**：已验证
- **日期**：`2026-09-03`
- **决策**：第 65 轮接入 `/simi/mv?mvid=`。URL 成功后异步拉相关 MV，失败不挡住播放。过滤当前 id。同一 id 若 URL 已缓存而 `relatedMvs` 仍为 `null`，再试一次。网格复用 MvCard。播放条背景保持 `#172033`。
- **原因**：legacy 详情页有「大家都在看」占位但没拉数据；MvCard 和 `/mv/detail` 已经能用。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、歌单行歌手 testid 对齐、评论。

### D-076：视频详情页用 `/video/detail` 补标题和作者

- **状态**：已验证
- **日期**：`2026-09-03`
- **决策**：第 66 轮接入 `/video/detail?id=`。URL 成功后异步拉详情，失败不挡住播放、不写 `error`。大厅缓存优先于详情。同一 id 若 URL 已缓存而详情为空，再试一次详情。播放条背景保持 `#172033`。
...
### D-078：歌曲行歌手对齐 `song-artist`

- **状态**：已验证
- **日期**：`2026-09-05`
- **决策**：第 68 轮把 `PlaylistSongItem` 歌手链接对齐新歌卡片/队列/播放条：正整数 `artist.id` 才进 `#/artistDetail`，带 `data-testid="song-artist"`、`aria-label` 和 `@click.stop`，放在播放按钮外面。播放条背景保持 `#172033`。
- **原因**：歌单/专辑/歌手/搜索歌曲行共用该组件；歌手入口早就有，但门槛和 testid 弱于其它表面。D-077 把它后置到本轮。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、专辑页头歌手 testid、用户主页链接。

### D-079：专辑页头歌手对齐 `song-artist`

- **状态**：已验证
- **日期**：`2026-09-05`
- **决策**：第 69 轮把 `AlbumHeader` 歌手链接对齐歌曲行：正整数 `artist.id` 才进 `#/artistDetail`，带 `data-testid="song-artist"`、`aria-label` 和 `@click.stop`，放在「播放全部」外面。播放条背景保持 `#172033`。
- **原因**：D-078 把专辑页头歌手 testid 后置；页头入口早就有，但门槛和 testid 弱于歌曲行。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-080：歌单详情页用 `/related/playlist` 接相关歌单

- **状态**：已验证
- **日期**：`2026-09-05`
- **决策**：第 70 轮接入 `/related/playlist?id=`。详情和歌曲成功后异步拉相关歌单，失败不挡住列表。过滤当前 id。同一 id 若歌单已缓存而 `relatedPlaylists` 仍为 `null`，再试一次。网格复用 CategoryPlaylistCard。播放条背景保持 `#172033`。
- **原因**：相关 MV / 相关视频已经用同一模式接过；歌单页还没有相关区。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-081：搜索建议解开 `mvs` 打开 MV 详情

- **状态**：已验证
- **日期**：`2026-09-05`
- **决策**：第 71 轮继续用 `/search/suggest`，把原来丢掉的 `result.mvs` 接到搜索页和顶栏弹出层。正整数 id 才进 `#/mvDetail`。封面认 `cover` / `picUrl` / `imgurl16v9`。不另开 `/cloudsearch?type=1004`。播放条背景保持 `#172033`。
- **原因**：歌单/歌手/专辑已经从同一接口解开；MV 详情页已经能播。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接、电台搜索。

### D-082：搜索建议解开 `djRadios` 打开电台详情

- **状态**：已验证
- **日期**：`2026-09-05`
- **决策**：第 72 轮继续用 `/search/suggest`，把原来丢掉的 `result.djRadios` 接到搜索页和顶栏弹出层。正整数 id 才进 `#/djRadio`。封面只认 `picUrl`。不另开 `/cloudsearch`。播放条背景保持 `#172033`。
- **原因**：D-081 把电台搜索后置；`#/djRadio` 已经能打开详情。歌单/歌手/专辑/MV 已经从同一接口解开。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-083：歌手详情页用 `/simi/artist` 接相似歌手

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 73 轮接入 `/simi/artist?id=`。详情和热门歌曲成功后异步拉相似歌手，失败不挡住列表。过滤当前 id。同一 id 若歌手已缓存而 `relatedArtists` 仍为 `null`，再试一次。提交结果只看 `loadedId`，不跟歌曲 `loadMore` 的序号绑在一起。网格复用 ArtistHallCard。播放条背景保持 `#172033`。
- **原因**：相关 MV / 相关视频 / 相关歌单已经用同一模式接过；歌手页还没有相似区。`#/artistDetail` 已经能打开。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-084：专辑详情页用 `/artist/album` 接更多专辑

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 74 轮在 `#/album` 用已有 `GET /artist/album` 拉该歌手的其他专辑。详情和歌曲成功后异步请求，失败不挡住列表。过滤当前专辑 id 和非正整数 id。歌手 id 无效则列表为空且不请求。同一 id 若专辑已缓存而 `relatedAlbums` 仍为 `null`，再试一次。提交只看 `loadedId`。网格复用 ArtistAlbumCard。播放条背景保持 `#172033`。
- **原因**：相似歌手 / 相关歌单已经用同一模式接过；专辑页还没有更多专辑区。`#/album` 和 `/artist/album` 都已经存在。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-085：电台详情页用 `/dj/radio/hot` 接更多电台

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 75 轮在 `#/djRadio` 用已有 `GET /dj/radio/hot` 拉同类其他电台。`/dj/detail` 解开正整数 `categoryId`。详情成功后异步请求，失败不挡住节目。过滤当前电台 id 和非正整数 id。分类 id 无效则列表为空且不请求。同一 id 若电台已缓存而 `relatedRadios` 仍为 `null`，再试一次。提交只看 `radioLoadedId`，不跟节目分页或大厅 `radios` 混用。网格复用 DjRadioCard。播放条背景保持 `#172033`。
- **原因**：更多专辑已经用同一模式接过；电台页还没有更多电台区。`#/djRadio` 和 `/dj/radio/hot` 都已经存在。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-086：电台节目详情页用 `/dj/program` 接更多节目

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 76 轮在 `#/dj` 用已有 `GET /dj/program?rid=` 拉同一电台的其他节目。`/dj/program/detail` 解开正整数 `radio.id`。详情成功后异步请求，失败不挡住当前节目。过滤当前节目 id 和非正整数 id。电台 id 无效则列表为空且不请求。同一 id 若节目已缓存而 `relatedPrograms` 仍为 `null`，再试一次。提交只看节目 `loadedId`，不跟电台页 `radioProgramSerial` 或大厅 `programs` 混用。网格复用 DjProgramCard。播放条背景保持 `#172033`。
- **原因**：更多电台已经用同一模式接过；节目页还没有更多节目区。`#/dj` 和 `/dj/program` 都已经存在。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接、节目页头电台链接。

### D-087：节目页头电台名打开 `#/djRadio`

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 77 轮把 DjProgramHeader 的电台名接到已有 `radioId`。正整数 id 且有电台名才进 `#/djRadio`，带 `data-testid="dj-radio"`、`aria-label` 和 `@click.stop`，放在「播放节目」外面。无效 id 仍是文本。主播名不进用户页。播放条背景保持 `#172033`。
- **原因**：D-086 把节目页头电台链接后置；`radioId` 已经从 `/dj/program/detail` 解开，`#/djRadio` 已经能打开。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。
...
- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 78 轮把 PlaylistHeader 非空标签接到 `#/music/category?cat=`。`/playlist/detail` 先 trim 再丢掉空标签。分类页读 `query.cat` 并 `setCat`；分类条改分类时写回查询（全部则去掉 cat）。创建者不进用户页。链接带 `data-testid="playlist-tag"`、`aria-label` 和 `@click.stop`，放在「播放全部」外面。播放条背景保持 `#172033`。
- **原因**：节目页头电台已经用同一模式接过；歌单标签还是纯文本，分类页已经能按 cat 拉精品歌单。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。
...
- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 79 轮把 DjRadioHeader 的分类名接到已有 `categoryId`。正整数 id 且有分类名才进 `#/music/dj?cateId=`，带 `data-testid="dj-category"` 和 `aria-label`。大厅读 `query.cateId` 并 `setCate`，先于默认第一分类。分类条改分类时写回查询。主播名不进用户页。播放条背景保持 `#172033`。
- **原因**：歌单页头分类已经用同一模式接过；电台分类还是纯文本，大厅已经能按 cateId 拉电台。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。

### D-090：播放列表用 `/simi/song` 接相似歌曲

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 80 轮接入 `/simi/song?id=`。当前曲设定后异步拉相似歌曲，失败不挡住播放。去掉当前曲、非正整数 id 和空白名。最多 10 条。`relatedSongs === null` 表示再试，`[]` 表示完成。提交只看 `current.id`，不跟 `play()` 的序号绑在一起。抽屉展示「相似歌曲」，点播走已有 `playSong`，抽屉保持打开。Host `clear()` 清掉列表。播放条背景保持 `#172033`。
- **原因**：详情页相关区已经用同一模式接过；播放列表还没有相似歌曲。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。相似区被播放条挡住留到第 84 轮。

### D-091：视频大厅分类写入 URL

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 81 轮让 `#/video` 读 `query.groupId`。正整数才生效，否则全部视频。落地先按查询拉列表，不先打 `groupId=0`。选分类写查询；选全部视频清掉查询。watch 按 `Pages.video` 门控，避免 keepAlive 元数据下进详情时把大厅重置。播放条背景保持 `#172033`。
- **原因**：分类歌单 `cat` 和电台大厅 `cateId` 已经能深链；视频分类还只活在 store 里。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、歌手馆筛选 URL。

### D-092：歌手馆筛选写入 URL

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 82 轮让 `#/music/artist` 读 `area` / `type` / `initial`。只认 `ARTIST_AREAS` / `ARTIST_TYPES` / `ARTIST_INITIALS`。`area=0` 是「其他」，不是全部。默认 `-1` / `-1` / `'-1'` 不写进 URL。改一项保留其它项。空查询 token 回落默认，避免 `Number('') === 0`。`applyHallFilters` 一次请求。watch 按 `Pages.artist` 门控。播放条背景保持 `#172033`。
- **原因**：视频大厅分类已经用同一模式接过；歌手馆三个筛选项还只活在 store 里。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、用户主页链接。
...
- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 83 轮继续用 `/search/suggest`，解开 `result.videos`。非空字符串 `vid` 才进 `#/videoDetail`。标题认 `title` 再 `name`。封面认 `coverUrl` / `cover` / `picUrl`。最多 10 条。不另开 `/cloudsearch`。搜索页和顶栏弹出层共用 SearchHitList。播放条背景保持 `#172033`。
- **原因**：suggest 的 MV 和电台已经接过；视频字段还在丢掉。视频详情页已经能打开。
- **本轮不做**：`/cloudsearch` 分页、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-094：播放列表让出播放条

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 84 轮让 PlayerBar 把实测高度写到 `--player-bar-height`。条出现后才写，卸载后清掉。播放列表面板 `bottom` 用该变量，相似歌曲停在播放条上方。队列层仍 `z-index: 30`，播放条 `40`。播放条背景保持 `#172033`。
- **原因**：第 80 轮审查 MEDIUM：相似歌曲 `flex-shrink: 0` 贴在滚动列表下面，被播放条盖住。
- **本轮不做**：歌词面板让位、登录、Tailwind 4、CI、专辑空评论、播放条换色。

### D-095：歌词面板让出播放条

- **状态**：已验证
- **日期**：`2026-09-06`
- **决策**：第 85 轮让歌词面板复用 `--player-bar-height`，带 `data-above-player`。歌词层仍 `z-index: 30`。播放条背景保持 `#172033`。
- **原因**：第 84 轮审查 LOW：歌词面板仍 `bottom: 0`，末行会进播放条下面。
- **本轮不做**：登录、Tailwind 4、CI、专辑空评论、播放条换色、队列删歌。
